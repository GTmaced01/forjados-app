...
export function LeaderTeamView() {
  ...
  async function loadData() {
    ...
    setError('');
    try {
      const data = await listLeaderTeamMembers(profile);
      setMembers(data);
    } catch (err) {
      console.error(err);
  ...
  const filteredMembers = useMemo(() => {
    ...
    return members.filter((member) => `${member.display_name} ${member.email} ${member.phone || ''} ${(member.sectors || []).join(' ')}`.toLowerCase().includes(q));
  }, [members, search]);
    ...
    <div className="leader-team-page">
      <div className="admin-header">
        <div>
          ...
          <p className="muted">Veja os membros aprovados dos mesmos setores/equipes que você lidera.</p>
        </div>
      ...
      <section className="panel wide">
        <div className="section-header">
          <div>
            ...
            <p className="muted">Seus setores: {profile?.sectors?.join(', ') || 'Nenhum setor definido'}</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="empty-access-requests"><h4>Nenhum liderado encontrado</h4><p className="muted">Confira se os membros já foram aprovados e se estão nos seus setores.</p></div>
        ) : (
