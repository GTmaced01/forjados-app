-- Restringe o reinício geral à edição que o operador está visualizando.
drop function if exists public.forjados_reset_trail_routes_v1(uuid);
...
begin
  ...
  where plan.edition_id = p_edition_id
  ...
  where execution.edition_id = plan.edition_id
    ...
    and plan.edition_id = p_edition_id
    and (p_group_id is null or plan.group_id = p_group_id);
...
revoke all on function public.forjados_reset_trail_routes_v1(uuid, uuid)
  from public, anon;
grant execute on function public.forjados_reset_trail_routes_v1(uuid, uuid)
  to authenticated;

comment on function public.forjados_reset_trail_routes_v1(uuid, uuid) is
  'Remove a execução atual de um ou todos os grupos de uma edição e os devolve ao Campo, preservando as rotas ideais.';
